...
const Dashboard = ({ section = 'overview' }) => {
  ...
  const canEditPricing = ['admin', 'manager'].includes(lowerRole);
  ...
  useEffect(() => {
    if (section !== 'pricing' && lowerRole && ['driver', 'member', 'registered_driver', 'customer', 'staff'].includes(lowerRole)) {
      navigate('/parking-map');
    }
  }, [lowerRole, navigate, section]);
              ...
              <table className="w-full min-w-[1080px] border-collapse text-left">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100 dark:bg-slate-800 dark:border-slate-700">
                    ...
                    <th className="px-4 py-3 text-xs font-extrabold uppercase tracking-wider text-slate-500 dark:text-slate-300">Giá Giờ Đầu</th>
                    <th className="px-4 py-3 text-xs font-extrabold uppercase tracking-wider text-slate-500 dark:text-slate-300">Giá Giờ Sau</th>
