...
const getVehicleIconByTypeId = (typeId) => {
  const id = Number(typeId);
  if (id === 1) return bikeIcon;
  if (id === 2) return motorbikeIcon;
  return carIcon;
};
...
const LocateVehicle = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const data = result?.isSuccess ? result.data : (result?.data || result);

      let typeId = null;
      if (data?.floorId && data?.slotName) {
        try {
          const slots = await parkingService.getSlotsByFloor(data.floorId);
          const matchedSlot = Array.isArray(slots)
            ? slots.find((s) => s.slotName === data.slotName)
            : null;
          typeId = matchedSlot?.typeId ?? null;
        } catch {
          typeId = null;
        }
      }

      setResult({ ...data, typeId });
      setError('');
    } catch (err) {
      setResult(null);
  ...
  return (
    <section id="locate-vehicle" className="relative w-full max-w-full overflow-hidden bg-slate-50 px-4 py-16 text-slate-950 transition-colors duration-300 dark:bg-[#0a0918] dark:text-white sm:px-6">
      <div className="anim-drift pointer-events-none absolute left-[-6rem] top-[-6rem] h-72 w-72 rounded-full bg-indigo-600/5 blur-[90px] dark:bg-indigo-500/20 sm:-left-24 sm:h-96 sm:w-96" />
      <div className="anim-drift-slow pointer-events-none absolute bottom-[-6rem] right-[-6rem] h-72 w-72 rounded-full bg-emerald-500/5 blur-[90px] dark:bg-emerald-400/20 sm:-right-24 sm:h-96 sm:w-96" />
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-6xl items-center gap-10 lg:grid-cols-2 lg:gap-16">
          ...
          <div className="mx-auto mt-8 max-w-md space-y-4 lg:mx-0">
            {steps.map((s, i) => (
              <div key={s.title} className="flex min-w-0 items-start gap-4 text-left">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 dark:bg-[#171527] dark:ring-indigo-900/40">
        ...
        <Reveal delay={150} className="relative min-w-0 w-full">
          <div className="relative z-10 mx-auto w-full max-w-md rounded-[24px] border border-slate-200 bg-white p-5 shadow-[0_30px_80px_-30px_rgba(15,23,42,0.25)] transition-colors duration-300 dark:border-indigo-400/20 dark:bg-[#141225] dark:shadow-[0_30px_80px_-20px_rgba(99,102,241,0.25)] dark:[&_h3]:text-white dark:[&_label]:text-slate-500 dark:[&_p]:text-slate-400 sm:p-8">
            ...
            <form onSubmit={handleSearch} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-[14px] border-[1.5px] border-slate-200 bg-slate-50 px-4 py-3.5 text-center text-base font-extrabold uppercase tracking-widest text-slate-900 placeholder:text-slate-300 transition-all duration-200 focus:scale-[1.01] focus:border-indigo-600 focus:outline-none focus:ring-4 focus:ring-indigo-600/20 dark:border-indigo-900/40 dark:bg-[#1e1b33] dark:text-white dark:placeholder:text-slate-500 sm:text-lg"
                />
              </div>
              <button
                type="submit"
            ...
            {error && (
              <div className="anim-fade-up-in mt-5 flex items-center justify-center gap-2 rounded-[14px] border border-red-200 bg-red-50 p-3.5 text-sm font-semibold text-red-600 dark:border-red-500/40 dark:bg-red-500/10 dark:text-red-300">
            ...
            {result && (
              <div className="anim-fade-up-in mt-6 space-y-4 border-t border-slate-100 pt-6 dark:border-indigo-900/40">
                <div className="text-center">
                  ...
                  <h3 className="flex min-w-0 items-center justify-center gap-2 break-all text-xl font-extrabold tracking-widest text-slate-900 dark:text-white sm:text-2xl">
                    <img src={getVehicleIconByTypeId(result.typeId)} alt="" className="h-6 w-6 object-contain" />
                    {result.licenseVehicle}
                  </h3>
                </div>

                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div className="rounded-[14px] border border-slate-200 bg-slate-50 p-4 dark:border-indigo-900/40 dark:bg-[#1e1b33]">
                ...
                {result.checkInImageUrl && (
                  <div className="dark:[&>span]:text-slate-500">
                    ...
                    <div className="group relative overflow-hidden rounded-[14px] border border-slate-200 shadow-sm dark:border-indigo-900/40">
                      <img
